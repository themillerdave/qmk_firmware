...
static uint16_t read_cols(void) {
  ...
  // reads the cols - hte right shift is to account for the starting pin for the cols starts at pin 20 (vs. 17, which is the first pin on the second I2C slave)
  return ~((((state_2) << 8) | state_1) >> 3);
}
...
void custom_matrix_init(void) {
  //init the rows on the I2C slave 1
  pca9555_init(IC1);
  //init the cols on thr I2c slave 2
  pca9555_init(IC2);
